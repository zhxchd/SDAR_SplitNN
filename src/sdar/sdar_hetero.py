...
class SDARHeteroAttacker:
    def __init__(self, client_ds_list, server_ds_list, num_class=10, batch_size=128) -> None:
        self.num_client = len(client_ds_list)
        self.client_ds_list = client_ds_list
        self.server_ds_list = server_ds_list
        self.num_class = num_class
        self.batch_size = batch_size
        self.client_dataset_list = [None] * self.num_client
        self.server_dataset_list = [None] * self.num_client
        for i in range(self.num_client):
            self.client_dataset_list[i] = client_ds_list[i].repeat(-1).batch(batch_size, drop_remainder=True)
            self.server_dataset_list[i] = server_ds_list[i].repeat(-1).batch(batch_size, drop_remainder=True)
    ...
    def run(self, level, num_iter, config, fg_lr=0.001, u_shape=False, conditional=True, use_e_dis=True, use_d_dis=True, model_type="resnet", width="standard", diff_simulator=False, dropout=0.0, l1=0.0, l2=0.0, alpha=0.0, verbose_freq=100):
        ...
        # Initialize models
        self.input_shape = self.client_ds_list[0].element_spec[0].shape
        ...
        for i in range(num_iter):
            ...
            x, y = next(iter(self.client_dataset_list[i % self.num_client]))
            xs, ys = next(iter(self.server_dataset_list[i % self.num_client]))
    ...
    def evaluate(self, eval_ds, verbose=False):
        eval_ds = eval_ds.batch(self.batch_size, drop_remainder=True)
