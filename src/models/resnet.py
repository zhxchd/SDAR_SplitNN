'''
Here we implement the models under resnet20 for 32x32 image input.
The model structure exactly follows Kaiming He's original ResNet paper on CIFAR.
...
def basic_block(x, planes, stride=1, downsample=None, name=None, l1=0.0, l2=0.0):
    identity = x
...
def make_f(level, input_shape, width="standard", l1=0.0, l2=0.0):
    ...
    if level >= 4:
        downsample_layer2 = [
            layers.Conv2D(filters=widths[1], kernel_size=1, strides=2, use_bias=False, name='layer2.0.downsample.0', kernel_regularizer=L1L2(l1=l1, l2=l2) if (l1 != 0.0 or l2 != 0.0) else None),
            layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name='layer2.0.downsample.1'),
        ]
        x = basic_block(x, widths[1], stride=2, downsample=downsample_layer2, name='layer2.0', l1=l1, l2=l2)
    if level >= 5:
    ...
    if level >= 7:
        downsample_layer3 = [
            layers.Conv2D(filters=widths[2], kernel_size=1, strides=2, use_bias=False, name='layer3.0.downsample.0', kernel_regularizer=L1L2(l1=l1, l2=l2) if (l1 != 0.0 or l2 != 0.0) else None),
            layers.BatchNormalization(momentum=0.9, epsilon=1e-5, name='layer3.0.downsample.1'),
        ]
        x = basic_block(x, widths[2], stride=2, downsample=downsample_layer3, name='layer3.0', l1=l1, l2=l2)
    if level >= 8:
